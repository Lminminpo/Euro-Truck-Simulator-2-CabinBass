...
def _key_display_name(key):
    ...
    name_map = {
        ...
        keyboard.Key.enter: "回车",
        keyboard.Key.shift: "Shift",
        keyboard.Key.shift_r: "R-Shift",
        keyboard.Key.ctrl: "Ctrl",
        keyboard.Key.ctrl_r: "R-Ctrl",
        keyboard.Key.alt: "Alt",
        keyboard.Key.alt_r: "R-Alt",
        keyboard.Key.tab: "Tab",
        keyboard.Key.caps_lock: "CapsLock",
        keyboard.Key.backspace: "退格",
...
class SettingsDialog:
    PARAMS = [
        ('bass',        '低音增强',      -0.50,  0.80,  0.01),
        ('freq',        '高频截止 (Hz)',  1000, 20000,   100),
        ('reflection',  '混响强度',       0.00,  1.00,  0.01),
        ('saturation',  '温暖度',         1.00,  1.50,  0.01),
        ('volume',      '音量',           0.10,  1.50,  0.01),
    ]
    ...
    def __init__(self, parent, current_settings):
        ...
        tk.Label(self.win, text="调整关窗和开窗的声学参数", font=("Arial", 10), fg="gray").pack(pady=(10, 5))
        ...
        # 关窗参数
        closed_frame = tk.LabelFrame(self._scroll_frame, text="关窗参数", font=("Arial", 10, "bold"))
        ...
        # 开窗参数
        open_frame = tk.LabelFrame(self._scroll_frame, text="开窗参数", font=("Arial", 10, "bold"))
...
class KeyBindDialog:
    ...
    def __init__(self, parent, config_path):
        ...
        self.lbl_title = tk.Label(self.win, text="欢迎使用 CabinBass", font=("Arial", 16, "bold"))
        self.lbl_title.pack(pady=(25, 5))

        self.lbl_sub = tk.Label(self.win, text="首次使用，请设置窗户控制按键", font=("Arial", 10), fg="gray")
        ...
        self.lbl_hint = tk.Label(self.win, text="请在键盘上按下对应按键", font=("Arial", 9), fg="gray")
        self.lbl_hint.pack()

        self.btn_skip = tk.Button(self.win, text="跳过（使用默认按键）", font=("Arial", 9),
                                  command=self._on_skip, relief=tk.FLAT, fg="gray")
        self.btn_skip.pack(side=tk.BOTTOM, pady=(0, 12))
    ...
    def _tk_event_to_config(self, event):
        ...
        modifier_keysyms = {
            ...
            'Scroll_Lock', 'Win_L', 'Win_R', 'Meta_L', 'Meta_R'
        }
        ...
        keysym_map = {
            'space':     ('vk:space',     '空格'),
            'Return':    ('vk:enter',     '回车'),
            'Shift_L':   ('vk:shift',     'Shift'),
            'Shift_R':   ('vk:shift_r',   'R-Shift'),
            'Control_L': ('vk:ctrl',      'Ctrl'),
            'Control_R': ('vk:ctrl_r',    'R-Ctrl'),
            'Alt_L':     ('vk:alt',       'Alt'),
            'Alt_R':     ('vk:alt_r',     'R-Alt'),
            'Tab':       ('vk:tab',       'Tab'),
            'Caps_Lock': ('vk:caps_lock', 'CapsLock'),
            'BackSpace': ('vk:backspace', '退格'),
            'Escape':    ('vk:esc',       'Esc'),
            'Up':        ('vk:up',        '↑'),
            'Down':      ('vk:down',      '↓'),
            'Left':      ('vk:left',      '←'),
            'Right':     ('vk:right',     '→'),
            'Insert':    ('vk:insert',    'Insert'),
            'Delete':    ('vk:delete',    'Delete'),
            'Home':      ('vk:home',      'Home'),
            'End':       ('vk:end',       'End'),
            'Prior':     ('vk:page_up',   'PageUp'),
            'Next':      ('vk:page_down', 'PageDown'),
        }
        if keysym in keysym_map:
            return keysym_map[keysym]
        if keysym.startswith('F') and keysym[1:].isdigit():
            fname = keysym.lower()
            return "vk:" + fname, fname.upper()
    ...
    def _next_step(self):
        ...
        if self._step >= len(self.STEPS):
            ...
            return
        self._update_step_text()
        self.lbl_key.config(text="等待按键...", fg="#3a7")
        self.win.focus_force()
    ...
    def _finish(self):
        ...
        self._done = True
        result = {}
        for _, field in self.STEPS:
            result[field] = self._keys[field]
...
class App:
    ...
    def _save_config(self):
        try:
            ...
            with open(self._config_path(), "w", encoding="utf-8") as f:
                ...
                f.write("settings_v1\n")
                for key in ('closed_bass', 'open_bass', 'closed_freq', 'open_freq',
                            'closed_reflection', 'open_reflection', 'closed_saturation',
                            'open_saturation', 'closed_volume', 'open_volume'):
            ...
            if lines and lines[0].strip() == "keybinds_v1":
                ...
                if len(lines) > settings_start and lines[settings_start].strip() == "settings_v1":
                    for i in range(settings_start + 1, len(lines)):
                        ...
                        if "=" in line:
                            k, v = line.split("=", 1)
                            if k in self._cab_settings:
                                try:
                                    if k in ('closed_freq', 'open_freq'):
    ...
    def _build_gui(self):
        self.root = tk.Tk()
        self.root.title("CabinBass v1.1")
