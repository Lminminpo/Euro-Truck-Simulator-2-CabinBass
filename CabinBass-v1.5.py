...
class CabinChannel:
    ...
    def _build_air_filter(self, freq):
        nyq = SAMPLE_RATE / 2
        f = max(3000, min(freq, nyq - 1)) / nyq
        return butter(1, f, btype='high')
    ...
    def process(self, data):
        ...
        air_component, self.air_zi = lfilter(self.air_b, self.air_a, out, zi=self.air_zi)
        out = out + air_component * 0.12
    ...
    def __init__(self, parent, current_settings, on_change=None):
        ...
        for i, (key, label, lo, hi, res) in enumerate(self.GLOBAL_PARAMS):
            ...
            tk.Scale(global_frame, from_=lo, to=hi, resolution=res, orient=tk.HORIZONTAL,
                     ...
                     length=300, command=lambda v, k=key: self._on_slider(k)).grid(row=i, column=1, padx=(0, 10), pady=2, sticky='ew')
        ...
        for i, (key, label, lo, hi, res) in enumerate(self.PARAMS):
            ...
            tk.Scale(closed_frame, from_=lo, to=hi, resolution=res, orient=tk.HORIZONTAL,
                     ...
                     length=300, command=lambda v, k=var_key: self._on_slider(k)).grid(row=i, column=1, padx=(0, 10), pady=2, sticky='ew')
...
class App:
    ...
    def _build_gui(self):
        self.root = tk.Tk()
        self.root.title("CabinBass v1.5")
    ...
    def _input_cb(self, indata, frames, time_info, status):
        ...
        if self._live_dev_sr != SAMPLE_RATE:
            g = gcd(self._live_dev_sr, SAMPLE_RATE)
            up = SAMPLE_RATE // g
            down = self._live_dev_sr // g
            ready = resample_poly(chunk, up, down, axis=0).astype(np.float32)
        else:
            ready = chunk
