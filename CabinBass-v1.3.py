...
class KeyBindDialog:
    ...
    def __init__(self, parent, config_path):
        ...
        tk.Label(self.win, text="首次使用，请设置窗户控制按键", font=("Arial", 10), fg="gray").pack(pady=(0, 20))
...
class App:
    ...
    def _build_gui(self):
        self.root = tk.Tk()
        self.root.title("CabinBass v1.3")
        ...
        tk.Label(self._live_frame,
                 text="使用前请先安装 VB-Audio Virtual Cable（免费）",
                 font=("Arial", 10, "bold")).pack(pady=(15, 2), anchor='w')

        guide = (
            "① 安装 VB-Cable 后重启电脑\n"
            "② 把音乐软件的「输出设备」改为 CABLE Input\n"
            "③ 在下方选择 CABLE Output，点击「开始监听」"
        )
        tk.Label(self._live_frame, text=guide, font=("Arial", 9), fg="gray",
                 justify=tk.LEFT).pack(pady=(0, 5), anchor='w')

        tk.Button(self._live_frame, text="下载 VB-Cable（官网）",
                  command=lambda: webbrowser.open("https://vb-audio.com/Cable/"),
                  font=("Arial", 9), fg="#06c", cursor="hand2",
                  relief=tk.FLAT, anchor='w').pack(pady=(0, 10), anchor='w')
